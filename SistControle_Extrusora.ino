...
#define RELAY2 25            // Cooler para a zona 1
#define RELAY3 24            // Cooler para a zona 2
#define RELAY4 23            // Cooler para a zona 3
#define RELAY5 26            // Cooler para o filamento
#define RELAY6 27            // Reserva
#define RELAY7 28            // Reserva
#define RELAY8 29            // Motor Fuso
#define BUZZER 41            // Pino conectado ao buzzer
#define BOTAO_EMERGENCIA 42  // Pino conectado ao EMG
...
MAX6675 thermocouple3(sckPin3, csPin3, soPin3);  // Configuração dos pinos MAX6675 3

bool respostaSim = false;  // Resposta do usuário na tela de confirmação
bool alarme_buzzer = true;
bool telaDesenhadaFSMAXZ1 = false;
bool telaDesenhadaFSMAXZ2 = false;
bool telaDesenhadaFSMAXZ3 = false;
bool telaDesenhadaFalhaAquecimento = false;
bool telaDesenhadaEmergencia = false;
bool telaEstaticaExibida = false;
bool produzindo = false;                     // false = Aquecendo, true = Produzindo
...
void setup() {
  ...
  pinMode(BOTAO_EMERGENCIA, INPUT_PULLUP);  // Configura como entrada com pull-up
  ...
  digitalWrite(BUZZER, HIGH);               // Garante que o buzzer inicie desligado
  digitalWrite(pinCS, HIGH);                // CS começa desativado
  digitalWrite(pinUD, LOW);                 // U/D inicializado em LOW
...
void coletaDados() {
  ...
  delay(1000);  // Atraso de 5 segundo entre as leituras
}
...
void loop() {
  ...
  if (!alarme_buzzer) {
    digitalWrite(BUZZER, LOW);   // Liga o buzzer (nível baixo)
    delay(500);                  // Aguarda 1 segundo
    digitalWrite(BUZZER, HIGH);  // Desliga o buzzer (nível alto)
...
void verificarBotoesModoAutomatico() {
  ...
  if (digitalRead(next) == LOW) {  // Se o botão de voltar for pressionado
    page = 17;
    telaSelecaoMemoriaAutomatico();  // Exibe a tela de Seleção Memória modo Automático
  }
  if (digitalRead(before) == LOW) {  // Se o botão de avançar for pressionado
...
void gravarNaEEPROM() {
  ...
  EEPROM.write(4, tempMinZ1);              // Temperatura mínima Z1 (endereço 4 a 6)
  EEPROM.write(7, tempMaxZ1);              // Temperatura máxima Z1 (endereço 7 a 9)
  EEPROM.write(10, tempMinZ2);             // Temperatura mínima Z2 (endereço 10 a 12)
  EEPROM.write(13, tempMaxZ2);             // Temperatura máxima Z2 (endereço 13 a 15)
  EEPROM.write(16, tempMinZ3);             // Temperatura mínima Z3 (endereço 16 a 18)
  EEPROM.write(19, tempMaxZ3);             // Temperatura máxima Z3 (endereço 19 a 21)
  telaConfirmacao();
}
...
void configurarPotenciometroDigital(int valor) {
  if (valor < 0) valor = 0;
  if (valor > 99) valor = 99;

  digitalWrite(pinCS, LOW);  // Ativa o potenciômetro

  for (int i = 0; i < valor; i++) {
    digitalWrite(pinUD, HIGH);  // Define direção (HIGH = Incrementa)
    delayMicroseconds(1);       // Pulso curto para U/D
    digitalWrite(pinUD, LOW);   // Finaliza pulso
    delay(1);
  }

  digitalWrite(pinCS, HIGH);  // Desativa o potenciômetro
}

void configurarMotorFuso(int velocidadeFuso) {
  if (velocidadeFuso >= 0 && velocidadeFuso <= 255) {
    analogWrite(pinPWMfuso, velocidadeFuso);  // Aplica o PWM
  }
}
...
void atualizarValoresTelaExtrusao() {
  ...
  // Normalização para as temperaturas (escala até larguraMaxTemp)
  int barraTempZ1 = map(tempZ1, 0, 300, 0, larguraMaxTemp);  // Exemplo: 0 a 300 °C
  int barraTempZ2 = map(tempZ2, 0, 300, 0, larguraMaxTemp);
  int barraTempZ3 = map(tempZ3, 0, 300, 0, larguraMaxTemp);

  // Normalização para as velocidades (escala até larguraMaxVel)
  int barraVelFuso = map(velocidadeFuso, 0, 255, 0, larguraMaxVel);  // Exemplo: 0 a 255
  int barraVelBobina = map(velocidadeBobinaAtualizado, 0, 255, 0, larguraMaxVel);

  // Temperatura Zona 1
  myGLCD.setColor(255, 0, 0);
  myGLCD.fillRect(20, 110, 20 + barraTempZ1, 130);
  myGLCD.setColor(255, 255, 255);
  myGLCD.print(String(tempZ1) + " C", 20, 140);

  // Temperatura Zona 2
  myGLCD.setColor(255, 0, 0);
  myGLCD.fillRect(180, 110, 180 + barraTempZ2, 130);
  myGLCD.setColor(255, 255, 255);
  myGLCD.print(String(tempZ2) + " C", 180, 140);

  // Temperatura Zona 3
  myGLCD.setColor(255, 0, 0);
  myGLCD.fillRect(340, 110, 340 + barraTempZ3, 130);
  myGLCD.setColor(255, 255, 255);
  myGLCD.print(String(tempZ3) + " C", 340, 140);

  // Velocidade do Fuso
  myGLCD.setColor(0, 255, 0);
  myGLCD.fillRect(20, 230, 20 + barraVelFuso, 250);
  myGLCD.setColor(255, 255, 255);
  myGLCD.print(String(velocidadeFuso), 20, 255);

  // Velocidade do Bobinamento
  myGLCD.setColor(0, 255, 0);
  myGLCD.fillRect(260, 230, 260 + barraVelBobina, 250);
  myGLCD.setColor(255, 255, 255);
  myGLCD.print(String(velocidadeBobinaAtualizado), 260, 255);
}

// Função responsável por controlar o processo de extrusão dentro de um loop
void processoExtrusao() {

  if (!telaEstaticaExibida) {
    telaExtrusaoEstatica();      // Exibe a tela estática apenas uma vez
    telaEstaticaExibida = true;  // Marca que a tela foi exibida
  }
  do {

    verificarEmergencia();  // Verifica o botão de emergência
    if (startTimer == 0) {
    ...
    tempZ1 = int(thermocouple1.readCelsius());  // Lê a temperatura do Sensor 1
    tempZ2 = int(thermocouple2.readCelsius());  // Lê a temperatura do Sensor 2
    tempZ3 = int(thermocouple3.readCelsius());  // Lê a temperatura do Sensor 3
    ...
    if ((tempZ1 >= tempMinZ1) && (tempZ2 >= tempMinZ2) && (tempZ3 >= tempMinZ3)) {
      ...
      int velocidadeFuso = map(analogRead(A0), 0, 1023, 0, 99);  // Simula leitura de entrada
      configurarPotenciometroDigital(velocidadeFuso);
      ...
      if (tempZ2 < tempMaxZ2) {
        digitalWrite(RELAY3, HIGH);  // Desliga Refrigeração Z1
      }
...
if (tempZ3 < tempMaxZ3) {
        digitalWrite(RELAY4, HIGH);  // Desliga Refrigeração Z1
      }
      if (tempZ3 <= tempMinZ3) {
        if (timer == false) {
          if (millis() - startTimer > endTimer) {
    ...
    if (digitalRead(select) == LOW) {  // Verifica se o botão "Next" foi pressionado
      telaConfirmacaoSaida();          // Exibe a tela de confirmação
      if (respostaSim) {               // Se o usuário confirmar
        saida = 1;                     // Configura para sair do loop
        page = 20;
        telaEstaticaExibida = false;
      } else {
        myGLCD.clrScr();
        telaEstaticaExibida = false;
        telaExtrusaoEstatica();  // Exibe a tela estática
      }
    }

    coletaDados();
  } while (saida < 1);  // Loop contínuo até a condição ser maior que 1

  delay(500);  // Aguarda 0,5 segundo antes de atualizar novamente
}

void telaFSMaxZ1() {
  if (!telaDesenhadaFSMAXZ1) {
    ...
    telaDesenhadaFSMAXZ1 = true;                           // Marca que a tela foi desenhada
  }
}

void telaFSMaxZ2() {
  if (!telaDesenhadaFSMAXZ2) {
    myGLCD.clrScr();                  // Limpa a tela
    myGLCD.setBackColor(255, 0, 0);   // Cor de fundo vermelha
    myGLCD.setColor(255, 0, 0);       // Cor do texto
    myGLCD.fillRect(0, 0, 480, 320);  // Preenche a tela inteira com vermelho
    ...
    telaDesenhadaFSMAXZ2 = true;
  }
}

void telaFSMaxZ3() {
  if (!telaDesenhadaFSMAXZ3) {
    myGLCD.clrScr();                  // Limpa a tela
    myGLCD.setBackColor(255, 0, 0);   // Cor de fundo vermelha
    myGLCD.setColor(255, 0, 0);       // Cor do texto
    myGLCD.fillRect(0, 0, 480, 320);  // Preenche a tela inteira com vermelho
    ...
    telaDesenhadaFSMAXZ3 = true;
  }
}

void telaFalhaAquecimento() {
  if (!telaDesenhadaFalhaAquecimento) {
    myGLCD.clrScr();                  // Limpa a tela
    myGLCD.setBackColor(255, 0, 0);   // Cor de fundo vermelha
    myGLCD.setColor(255, 0, 0);       // Cor do texto
    myGLCD.fillRect(0, 0, 480, 320);  // Preenche a tela inteira com vermelho
    ...
    telaDesenhadaFalhaAquecimento = true;
  }
}
...
void telaConfirmacaoSaida() {
  ...
  myGLCD.clrScr();
  myGLCD.setBackColor(0, 0, 0);                 // Limpa a tela
  myGLCD.setColor(250, 250, 250);               // Define a cor do texto
  ...
  // Aguardar resposta
  while (true) {
    if (digitalRead(next) == LOW) {  // Botão Next pressionado
      respostaSim = true;
      break;
    } else if (digitalRead(before) == LOW) {  // Botão Before pressionado
      respostaSim = false;
      break;
    }
  }
...
void desligarSistema() {
  ...
  digitalWrite(RELAY2, LOW);    // Liga Refrigeração Z1
  digitalWrite(RELAY3, LOW);    // Liga Refrigeração Z2
  digitalWrite(RELAY4, LOW);    // Liga Refrigeração Z3
  digitalWrite(RELAY5, LOW);    // Liga Refrigeração Z4 saída filamento
  digitalWrite(pinSSR1, HIGH);  // Desliga Resistência 1
  digitalWrite(pinSSR2, HIGH);  // Desliga Resistência 2
  ...
  if ((tempZ1 <= 50) && (tempZ2 <= 50) && (tempZ3 <= 50)) {
    digitalWrite(RELAY1, HIGH);    // Desliga Refrigeração Alimentação
    digitalWrite(RELAY2, HIGH);    // Desliga Refrigeração Z1
    digitalWrite(RELAY3, HIGH);    // Desliga Refrigeração Z2
    digitalWrite(RELAY4, HIGH);    // Desliga Refrigeração Z3
    page = 1;                      // Passa para a tela de pergunta sobre o modo automático
    telaPerguntaModoAutomatico();  // Exibe a tela de pergunta sobre o modo automático
  }
...
void alarmes() {
  // Verifica se o botão "Select" foi pressionado para iniciar a saída (Sim)
...
void verificarEmergencia() {
  if (digitalRead(BOTAO_EMERGENCIA) == HIGH) {  // Botão pressionado
    desligarEmergencia();                       // Função para desligar o sistema de forma segura
    telaEmergencia();

    while (true) {                 // Loop infinito para garantir a parada do sistema
      digitalWrite(BUZZER, LOW);   // Liga o buzzer (nível baixo)
      delay(500);                  // Aguarda 1 segundo
      digitalWrite(BUZZER, HIGH);  // Desliga o buzzer (nível alto)
      delay(300);                  // Aguarda 1 segundo  // Loop infinito para garantir a parada do sistema
    }
  }
}
...
void desligarEmergencia() {
  ...
  digitalWrite(RELAY4, HIGH);   // Desliga Refrigeração Z3
  digitalWrite(RELAY8, HIGH);   // Desliga motor do fuso
  digitalWrite(pinSSR1, HIGH);  // Desliga a resistência 1
  digitalWrite(pinSSR2, HIGH);  // Desliga a resistência 2
}

void telaEmergencia() {
  if (!telaDesenhadaEmergencia) {
    myGLCD.clrScr();                  // Limpa a tela
    myGLCD.setBackColor(255, 0, 0);   // Cor de fundo vermelha
    myGLCD.setColor(255, 0, 0);       // Cor do texto
    myGLCD.fillRect(0, 0, 480, 320);  // Preenche a tela inteira com vermelho
    ...
    myGLCD.setColor(255, 255, 255);           // Define a cor do texto para branco
    myGLCD.setFont(BigFont);                  // Define uma fonte grande
    myGLCD.print("EMERGENCIA", CENTER, 240);  // Mensagem principal
    myGLCD.print("ATUADO", CENTER, 270);      // Mensagem secundária
    telaDesenhadaEmergencia = true;
    alarme_buzzer = false;
  }
}